...
Recall that a random variable $X$ is said to be normally distirbuted if its density function is 

$$
p(x) = \frac{1}{\sqrt{2\pi\sigma^2}}\exp(-\frac{1}{2\sigma^2}(x-\theta)^2)\qquad x\in \mathbb{R}
$$

where $\theta = \mathbb{E}(X)$ and $\sigma^2 = \mathbb{V}(X)$ are the mean and the variance respectively.


Let $\theta$ be the parameter of interest and suppose the model $X_1, \dots X_n |\theta,\sigma^2 \sim_{iid} \mathcal{N}(\theta, \sigma^2)$. Furthermore, lets assume, for the sake of simplicity, that $\sigma^2$ is known. This is unlikely in practice so we will treat the case of unknown variance as well.

Then, the likelihood function in this case is:

$$
p(x | \theta, \sigma^2) = p(x_1, \dots, x_n | \theta, \sigma^2) = \prod_{i=1}^n p(x_i|\theta, \sigma^2) = \frac{1}{(2\pi\sigma^2)^{n/2}}\exp\left(-\frac{1}{2\sigma^2}\sum_{i}(x_i -\theta)^2\right)
$$

Therefore, for any prior distribution on $\theta$  (conditional on $\sigma^2$) $p(\theta |\sigma^2)$, the posterior distribution is:

$$
\begin{align*}
p(\theta |x_1,\dots, x_n,\sigma^2) &= \frac{p(\theta|\sigma^2)p(x|\theta, \sigma^2)}{p(x_1,\dots, x_n)}\\
& \propto  p(\theta|\sigma^2) \exp(-\frac{1}{2}\sum_i(x_i - \theta)^2)
\end{align*}
$$
where the proportionality constant depends on the data $x$ and known parameters but no on $\theta$ itself.

Now, from the last expression above we see that if the prior distribution of $\theta$ is a normal, then the posterior will have the form of a quadratic exponential, that is, this posterior is going to be normal itself.



In fact, take as a prior for $\theta$ a normal $\mathcal{N}(\theta_0, \phi_0)$ with $\theta_0, \phi_0$ known quantities, then, the posterior is:

$$
p(\theta |x_1,\dots, x_n, \sigma^2) \propto \exp(-\frac{1}{2\phi_0^2}(\theta - \theta_0)^2)\exp(-\frac{1}{2}\sum_i(x_i - \theta)^2)
$$

where, again, the propotionality constant depend on the data and the known quantities $\sigma^2,\phi_0$.

Click <a href ="normalpriorlikelihood"> here </a> for a detailed derivation of the posterior distribution.

We need to identify the posterior mean and variance of $\theta$. For this, lets expand the terms inside the exponential functions:

$$
\begin{align*}
& \frac{1}{\phi_0^2}(\theta^2 - 2\theta_0\theta + \theta_0^2) + \frac{1}{\sigma^2}(\sum_i x_i^2 - 2\theta\sum_i x_i + n\theta^2)\\
&= a\theta^2 -2b\theta + c 
\end{align*}
$$

with $a = \frac{1}{\phi_0^2} + \frac{n}{\sigma^2}$, $b = \frac{1}{\phi_0^2}\theta_0 + \frac{n}{\sigma^2}\bar{x}$ and $c = c(\theta_0, \phi_0^2, \sigma^2, x_1,\dots, x_n)$

In order to make some sense of these quantities, recall that the precision of a random variable is defined as the inverse of its variance. Thus, $1/\phi_0^2$ is the prior precision of $\theta$. $n/\sigma^2$ is the precision of the sample mean $\bar{X}_n$, therefore, the constant $a$ defined above is simply the sum of these two precisions and we will rename it as:

$$
\frac{1}{\phi^2} =  \frac{1}{\phi_0^2} + \frac{n}{\sigma^2}
$$

Note that with this notation the term defined by $b$ above is a weighted average of the prior mean of $\theta$ and the sample mean of the data, with the weights being the prior and data precision.

Now the posterior distribution of the mean $\theta$ takes the form of a normal distribution:

$$
\begin{align*}
p(\theta \vert x_1, \dots, x_n, \sigma^2) & \propto \exp(-0.5(a\theta^2 -2b\theta)) \\
& = \exp\left(-0.5a(\theta^2 - 2b\theta/a +b^2/a^2) + 0.5b^2/a\right)
& \propto \exp(-\frac{1}{2}a(\theta - b/a)^2)
\end{align*}
$$

where the mean $\mu_n$ an the variance $\phi_n^2$ of this normal distribution are:

$$
\mu_n = \frac{b}{a} = \frac{\frac{1}{\phi_0^2} \theta_0 + \frac{n}{\sigma^2} \bar{x}}{\frac{1}{\phi_0^2} + \frac{n}{\sigma^2}} \qquad \phi_n^2 = \frac{1}{a} = \frac{1}{\frac{1}{\phi_0^2} + \frac{n}{\sigma^2}}
$$

Again, in order to get more intuition from these terms and have cleaner formulas let us rename the prior and data precision as $\tilde{\phi_0}^2, \tilde{\sigma}^2$ respectively. Then, the posterior mean and precision take the form:

$$
\begin{align*}
\mu_n &= \frac{\tilde{\phi_0}^2}{\tilde{\phi_0}^2 + n\tilde{\sigma}^2}\theta_0 + \frac{n\tilde{\sigma}^2}{\tilde{\phi_0}^2 + n\tilde{\sigma}^2}\bar{x}\\
\\
\tilde{\phi_n}^2 &= \tilde{\phi_0}^2 + n\tilde{\sigma}^2 
\end{align*}
$$
